...
confint.dimorphResampledUni <- function(x, conf.level=0.95, alternative="two.sided", type="estimate") {
  ...
  if (is.null(attr(x, "resampling"))) warning("This object was not generated by a bootstrap -\ntake care when interpreting these intervals.")
  if(type=="estimate") {
    #if (is.null(x$CI)) stop("There are no confidence intervals associated with this object.")
    #{if (!is.null(x$CI) & !is.null(attr(x, "conf.level")) & !is.null(attr(x, "alternative"))) if(attr(x, "conf.level")==conf.level & attr(x, "alternative")==alternative) {
	#  out <- x$CI
	#}
	#else {
	  if (!is.null(x$CI) & !is.null(attr(x, "conf.level"))) if (attr(x, "conf.level")!=conf.level) warning("These intervals are for a different confidence level than\noriginally calculated when the bootstrap was run.")
	  if (!is.null(x$CI) & !is.null(attr(x, "alternative"))) if (attr(x, "alternative")!=alternative) warning("These intervals are for a different value of 'alternative' than\noriginally calculated when the bootstrap was run.")
  ...
  if(type=="bias") {
	...
	#}
	#else {
	  if (!is.null(x$CIbias) & !is.null(attr(x, "conf.level"))) if (attr(x, "conf.level")!=conf.level)  warning("These intervals are for a different confidence level than\noriginally calculated when the bootstrap was run.")
	  if (!is.null(x$CIbias) & !is.null(attr(x, "alternative"))) if (attr(x, "alternative")!=alternative) warning("These intervals are for a different value of 'alternative' than\noriginally calculated when the bootstrap was run.")
