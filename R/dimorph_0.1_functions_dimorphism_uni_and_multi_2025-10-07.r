...
#'     Method (ARM) following Gordon (2025a).  ARM is a resampling-based estimate of dimorphism that repeatedly 
#'     samples two values
...
#'     of 1 whenever the same value is sampled twice and is not rejected by retention criterion.
...
#'   \item \code{"BFM"}: \bold{Bayesian Finite Mixture}.  Follows Gordon (2025a).  Assumes that the sample is a 
...
#'     univariate method is then applied to this measure of overall size.  If any specimens are 
#'     missing data, those specimens will be dropped from the analysis if \code{na.rm=TRUE}, or the 
#'     function will return \code{NA} if \code{na.rm=FALSE}.
#'   \item \code{"GMM"}: Follows the Geometric Mean Method of Gordon et al. (2008).  The
#'     selected univariate method is applied to all variables separately, then the geometric mean
...
#'     is specified by the user with the argument \code{templatevar}.  The algorithm identifies a
#'     template individual that can be used to estimate the largest number of values for the 
#'     selected variable of interest.  It does so by calculating ratios between the value of the 
#'     variable of interest and other variables in the template individual, which are then 
#'     multiplied by the value of those other variables in individuals missing the target variable. 
